...
from .spoofing_order_tracker import SpoofingOrderTracker
...
cdef class SpoofingStrategy(StrategyBase):
    ...
    def inventory_skew_stats_data_frame(self) -> Optional[pd.DataFrame]:
        ...
        inventory_skew_df = pd.DataFrame(data=[
            ...
            ["Order Adjust %", f"{bid_ask_ratios.bid_ratio:.1%}"]
        ])
        return inventory_skew_df

    def spoofing_assets_df(self, to_show_current_pct: bool) -> pd.DataFrame:
    ...
    def format_status(self) -> str:
        ...
        assets_df = self.spoofing_assets_df(not self._inventory_skew_enabled)
