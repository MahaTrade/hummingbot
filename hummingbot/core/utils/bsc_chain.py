...
def is_connected_to_web3(bsc_rpc_url: str) -> bool:
    ...
    """
    w3: Web3 = Web3(Web3.HTTPProvider(bsc_rpc_url, request_kwargs={"timeout": 2.0}))
    return w3.isConnected()


def check_web3(bsc_rpc_url: str) -> bool:
    """
    Confirm that the provided url is a valid bsc RPC url.
    """
    try:
        ret = is_connected_to_web3(bsc_rpc_url)
    except Exception:
        ret = False

    if not ret:
        if bsc_rpc_url.startswith("http://bsc-dataseed.binance.org"):
        ...
        if bsc_rpc_url.startswith("bsc-dataseed.binance.org"):
...
def check_transaction_exceptions(trade_data: dict) -> list:
    """
    Check trade data for Binance decentralized exchanges
    """
    exception_list = []
    ...
    bsc_balance = balances["BSC"]

    # check for sufficient gas
    if bsc_balance < gas_cost:
        exception_list.append(f"Insufficient BSC balance to cover gas:"
                              f" Balance: {bsc_balance}. Est. gas cost: {gas_cost}. {swaps_message}")
