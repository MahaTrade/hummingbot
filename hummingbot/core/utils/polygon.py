...
def is_connected_to_web3(polygon_rpc_url: str) -> bool:
    """
    This is abstracted out of check_web3 to make mock testing easier
    """
    w3: Web3 = Web3(Web3.HTTPProvider(polygon_rpc_url, request_kwargs={"timeout": 2.0}))
    return w3.isConnected()


def check_web3(polygon_rpc_url: str) -> bool:
    """
    Confirm that the provided url is a valid Ethereum RPC url.
    """
    try:
        ret = is_connected_to_web3(polygon_rpc_url)
    except Exception:
        ret = False

    if not ret:
        if polygon_rpc_url.startswith("http://mainnet.infura.io"):
        ...
        if polygon_rpc_url.startswith("mainnet.infura.io"):
...
async def get_token_list():
    ...
    """
    token_list_url = global_config_map.get("polygon_token_list_url").value
