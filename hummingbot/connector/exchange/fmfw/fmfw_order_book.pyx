...
from hummingbot.connector.exchange.fmfw.fmfw_order_book_message import FmfwOrderBookMessage

_kob_logger = None


cdef class FmfwOrderBook(OrderBook):
    @classmethod
    ...
    def diff_message_from_exchange(cls,
        ...
        if metadata:
            msg.update(metadata)
        return FmfwOrderBookMessage(OrderBookMessageType.DIFF, {
    ...
    def diff_message_from_db(cls, record: RowProxy, metadata: Optional[Dict] = None) -> OrderBookMessage:
        ts = int(record["timestamp"])
        msg = ujson.loads(record["json"])  # Fmfw json in DB is TEXT
        if metadata:
            msg.update(metadata)
