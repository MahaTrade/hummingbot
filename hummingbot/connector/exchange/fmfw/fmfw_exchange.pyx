...
from hummingbot.connector.exchange.fmfw.fmfw_api_order_book_data_source import FmfwAPIOrderBookDataSource
from hummingbot.connector.exchange.fmfw.fmfw_auth import FmfwAuth
from hummingbot.connector.exchange.fmfw.fmfw_in_flight_order import FmfwInFlightOrder
from hummingbot.connector.exchange.fmfw.fmfw_order_book_tracker import FmfwOrderBookTracker
from hummingbot.connector.exchange.fmfw.fmfw_user_stream_tracker import FmfwUserStreamTracker
...
cdef class FmfwExchangeTransactionTracker(TransactionTracker):
    cdef:
        FmfwExchange _owner

    def __init__(self, owner: FmfwExchange):
        super().__init__()
        self._owner = owner
...
cdef class FmfwExchange(ExchangeBase):
    ...
    def __init__(self,
        ...
        self._order_book_tracker = FmfwOrderBookTracker(trading_pairs, self._fmfw_auth)
        ...
        self._tx_tracker = FmfwExchangeTransactionTracker(self)
        self._user_stream_tracker = FmfwUserStreamTracker(fmfw_auth=self._fmfw_auth)
    ...
    @property
    def order_book_tracker(self) -> FmfwOrderBookTracker:
    ...
    @property
    def in_flight_orders(self) -> Dict[str, FmfwInFlightOrder]:
    ...
    def restore_tracking_states(self, saved_states: Dict[str, Any]):
        self._in_flight_orders.update({
            key: FmfwInFlightOrder.from_json(value)
    ...
    @property
    def user_stream_tracker(self) -> FmfwUserStreamTracker:
                ...
                if (execution_status == "done" or execution_status == "match") and (execution_type == "match" or execution_type == "filled"):
                    ...
                    if tracked_order.trade_type == TradeType.BUY:
                        self.logger().info(f"The market buy order {tracked_order.client_order_id} has completed "
                                           f"according to Fmfw user stream.")
    ...
    cdef object c_get_fee(self,
        ...
        """
        if order_type is OrderType.LIMIT and fee_overrides_config_map["fmfw_maker_fee"].value is not None:
            return TradeFee(percent=fee_overrides_config_map["fmfw_maker_fee"].value / Decimal("100"))
        if order_type is OrderType.MARKET and fee_overrides_config_map["fmfw_taker_fee"].value is not None:
            return TradeFee(percent=fee_overrides_config_map["fmfw_taker_fee"].value / Decimal("100"))
        return TradeFee(percent=Decimal("0.001"))
        """
        is_maker = order_type is OrderType.LIMIT_MAKER
        return estimate_fee("fmfw", is_maker)
    ...
    async def _update_trading_rules(self):
        ...
        if current_tick > last_tick or len(self._trading_rules) < 1:
            exchange_info = await self._api_request("get", path_url="/api/3/public/symbol")
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching account updates.",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch account updates from Fmfw. "
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching trading rules.",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch new trading rules from Fmfw. "
    ...
    async def place_order(self,
        ...
        return str(exchange_order_id["client_order_id"])
    ...
    async def execute_buy(self,
        ...
        try:
            ...
            self.logger().network(
                f"Error submitting buy {order_type_str} order to Fmfw for "
                f"{decimal_amount} {trading_pair} "
                f"{decimal_price}.",
                exc_info=True,
                app_warning_msg=f"Failed to submit buy order to Fmfw. Check API key and network connection."
            )
    ...
    async def execute_sell(self,
        ...
        try:
            ...
            order_type_str = order_type.name.lower()
            self.logger().network(
                f"Error submitting sell {order_type_str} order to Fmfw for "
                f"{decimal_amount} {trading_pair} "
                f"{decimal_price}.",
                exc_info=True,
                app_warning_msg=f"Failed to submit sell order to Fmfw. Check API key and network connection."
            )
            self.c_trigger_event(self.MARKET_ORDER_FAILURE_EVENT_TAG,
                                 MarketOrderFailureEvent(self._current_timestamp, order_id, order_type))
    ...
    async def execute_cancel(self, trading_pair: str, order_id: str):
        try:
            ...
            path_url = f"/api/3/spot/order/{tracked_order.exchange_order_id}"
            await self._api_request("delete", path_url=path_url, is_auth_required=True)
        except Exception as e:
            self.logger().network(
                f"Failed to cancel order {order_id}: {str(e)}",
                exc_info=True,
                app_warning_msg=f"Failed to cancel the order {order_id} on Fmfw. "
    ...
    async def cancel_all(self, timeout_seconds: float) -> List[CancellationResult]:
        ...
        try:
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg=f"Failed to cancel all orders on Fmfw. Check API key and network connection."
            )
    ...
    cdef c_start_tracking_order(self,
                                ...
                                object price,
                                object amount):
        self._in_flight_orders[client_order_id] = FmfwInFlightOrder(
