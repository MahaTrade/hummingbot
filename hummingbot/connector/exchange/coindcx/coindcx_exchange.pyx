...
from hummingbot.connector.exchange.coindcx.coindcx_in_flight_order import CoindcxInFlightOrder
from hummingbot.connector.exchange.coindcx.coindcx_order_book_tracker import CoindcxOrderBookTracker
from hummingbot.connector.exchange.coindcx.coindcx_user_stream_tracker import CoindcxUserStreamTracker
...
COINDCX_ROOT_API = "https://api.coindcx.com"


class CoindcxAPIError(IOError):
    def __init__(self, error_payload: Dict[str, Any]):
        super().__init__()
        self.error_payload = error_payload


cdef class CoindcxExchangeTransactionTracker(TransactionTracker):
    cdef:
        CoindcxExchange _owner

    def __init__(self, owner: CoindcxExchange):
        super().__init__()
        self._owner = owner
...
cdef class CoindcxExchange(ExchangeBase):
    ...
    def __init__(self,
        ...
        self._coindcx_auth = CoindcxAuth(api_key=coindcx_api_key,
                                       secret_key=coindcx_secret_key)
        self._in_flight_orders = {}
        self._last_poll_timestamp = 0
        self._last_timestamp = 0
        self._order_book_tracker = CoindcxOrderBookTracker(trading_pairs, self._coindcx_auth)
        ...
        self._tx_tracker = CoindcxExchangeTransactionTracker(self)
        self._user_stream_tracker = CoindcxUserStreamTracker(coindcx_auth=self._coindcx_auth)
    ...
    @property
    def order_book_tracker(self) -> CoindcxOrderBookTracker:
    ...
    @property
    def in_flight_orders(self) -> Dict[str, CoindcxInFlightOrder]:
    ...
    def restore_tracking_states(self, saved_states: Dict[str, Any]):
        self._in_flight_orders.update({
            key: CoindcxInFlightOrder.from_json(value)
    ...
    @property
    def user_stream_tracker(self) -> CoindcxUserStreamTracker:
        ...
        while True:
            try:
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg="Could not fetch user events from Coindcx. Check API key and network connection."
                )
    ...
    async def _user_stream_event_listener(self):
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                # Refer to https://docs.coindcx.com/#private-order-change-events
    ...
    async def _api_request(self,
        ...
        if is_auth_required:
                headers = self._coindcx_auth.add_auth_to_params(data)
        else:
    ...
    cdef object c_get_fee(self,
        ...
        # Fee info from https://www.coindcx.com/vip/fee
        """
        if order_type is OrderType.LIMIT and fee_overrides_config_map["coindcx_maker_fee"].value is not None:
            return TradeFee(percent=fee_overrides_config_map["coindcx_maker_fee"].value / Decimal("100"))
        if order_type is OrderType.MARKET and fee_overrides_config_map["coindcx_taker_fee"].value is not None:
            return TradeFee(percent=fee_overrides_config_map["coindcx_taker_fee"].value / Decimal("100"))
        return TradeFee(percent=Decimal("0.001"))
        """
        is_maker = order_type is OrderType.LIMIT_MAKER
        return estimate_fee("coindcx", is_maker)
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching account updates.",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch account updates from Coindcx. "
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching trading rules.",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch new trading rules from Coindcx. "
    ...
    async def execute_buy(self,
        ...
        try:
            ...
            self.logger().network(
                f"Error submitting buy {order_type_str} order to Coindcx for "
                f"{decimal_amount} {trading_pair} "
                f"{decimal_price}.",
                exc_info=True,
                app_warning_msg=f"Failed to submit buy order to Coindcx. Check API key and network connection."
            )
    ...
    async def execute_sell(self,
        ...
        try:
            ...
            order_type_str = order_type.name.lower()
            self.logger().network(
                f"Error submitting sell {order_type_str} order to Coindcx for "
                f"{decimal_amount} {trading_pair} "
                f"{decimal_price}.",
                exc_info=True,
                app_warning_msg=f"Failed to submit sell order to Coindcx. Check API key and network connection."
            )
            self.c_trigger_event(self.MARKET_ORDER_FAILURE_EVENT_TAG,
                                 MarketOrderFailureEvent(self._current_timestamp, order_id, order_type))
    ...
    async def execute_cancel(self, trading_pair: str, order_id: str):
        try:
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg=f"Failed to cancel the order {order_id} on Coindcx. "
    ...
    async def cancel_all(self, timeout_seconds: float) -> List[CancellationResult]:
        ...
        try:
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg=f"Failed to cancel all orders on Coindcx. Check API key and network connection."
            )
    ...
    cdef c_start_tracking_order(self,
                                ...
                                object price,
                                object amount):
        self._in_flight_orders[client_order_id] = CoindcxInFlightOrder(
