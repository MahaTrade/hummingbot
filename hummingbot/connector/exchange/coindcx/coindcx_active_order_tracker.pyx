...
CoindcxOrderBookTrackingDictionary = Dict[Decimal, Dict[str, Dict[str, any]]]

cdef class CoindcxActiveOrderTracker:
    def __init__(self,
                 active_asks: CoindcxOrderBookTrackingDictionary = None,
                 active_bids: CoindcxOrderBookTrackingDictionary = None):
        super().__init__()
    ...
    @property
    def active_asks(self) -> CoindcxOrderBookTrackingDictionary:
        return self._active_asks

    @property
    def active_bids(self) -> CoindcxOrderBookTrackingDictionary:
    ...
    cdef tuple c_convert_snapshot_message_to_np_arrays(self, object message):
        ...
        for snapshot_orders, active_orders in [(message.content["bids"], self._active_bids), (message.content["asks"], self.active_asks)]:
            for price , quantity in snapshot_orders.items():
                price = price
                amount = str(quantity)
                order_dict = {
                    "order_id": timestamp,
