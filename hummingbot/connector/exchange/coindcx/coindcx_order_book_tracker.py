...
from hummingbot.connector.exchange.coindcx.coindcx_order_book_message import CoindcxOrderBookMessage
from hummingbot.connector.exchange.coindcx.coindcx_active_order_tracker import CoindcxActiveOrderTracker


class CoindcxOrderBookTracker(OrderBookTracker):
    ...
    def __init__(self,
                 ...
                 auth: CoindcxAuth = None):
        super().__init__(CoindcxAPIOrderBookDataSource(trading_pairs, auth), trading_pairs)
        self._auth = auth
        self._order_book_diff_stream: asyncio.Queue = asyncio.Queue()
        self._order_book_snapshot_stream: asyncio.Queue = asyncio.Queue()
        self._ev_loop: asyncio.BaseEventLoop = asyncio.get_event_loop()
        self._saved_message_queues: Dict[str, Deque[CoindcxOrderBookMessage]] = defaultdict(lambda: deque(maxlen=1000))
        self._active_order_trackers: Dict[str, CoindcxActiveOrderTracker] = defaultdict(CoindcxActiveOrderTracker)
    ...
    async def _order_book_diff_router(self):
        ...
        while True:
            try:
                ob_message: CoindcxOrderBookMessage = await self._order_book_diff_stream.get()
    ...
    async def _track_single_book(self, trading_pair: str):
        past_diffs_window: Deque[CoindcxOrderBookMessage] = deque()
        self._past_diffs_windows[trading_pair] = past_diffs_window

        message_queue: asyncio.Queue = self._tracking_message_queues[trading_pair]
        order_book: OrderBook = self._order_books[trading_pair]
        active_order_tracker: CoindcxActiveOrderTracker = self._active_order_trackers[trading_pair]
        ...
        while True:
            try:
                message: CoindcxOrderBookMessage = None
                saved_messages: Deque[CoindcxOrderBookMessage] = self._saved_message_queues[trading_pair]
                ...
                if message.type is OrderBookMessageType.DIFF:
                    ...
                    past_diffs: List[CoindcxOrderBookMessage] = list(past_diffs_window)
