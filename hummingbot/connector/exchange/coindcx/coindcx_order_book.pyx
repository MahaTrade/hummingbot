...
cdef class CoindcxOrderBook(OrderBook):
    @classmethod
    ...
    def snapshot_message_from_exchange(cls,
        ...
        if metadata:
            msg.update(metadata)
        return CoindcxOrderBookMessage(OrderBookMessageType.SNAPSHOT, {
    ...
    def diff_message_from_exchange(cls,
        ...
        if metadata:
            msg.update(metadata)
        return CoindcxOrderBookMessage(OrderBookMessageType.DIFF, {
    ...
    def diff_message_from_db(cls, record: RowProxy, metadata: Optional[Dict] = None) -> OrderBookMessage:
        ts = int(record["timestamp"])
        msg = ujson.loads(record["json"])  # Coindcx json in DB is TEXT
        if metadata:
            msg.update(metadata)
