...
from hummingbot.connector.exchange.lbank.lbank_order_book_message import LbankOrderBookMessage
from hummingbot.connector.exchange.lbank.lbank_active_order_tracker import LbankActiveOrderTracker


class LbankOrderBookTracker(OrderBookTracker):
    ...
    def __init__(self,
                 ...
                 auth: LbankAuth = None):
        super().__init__(LbankAPIOrderBookDataSource(trading_pairs, auth), trading_pairs)
        self._auth = auth
        self._order_book_diff_stream: asyncio.Queue = asyncio.Queue()
        self._order_book_snapshot_stream: asyncio.Queue = asyncio.Queue()
        self._ev_loop: asyncio.BaseEventLoop = asyncio.get_event_loop()
        self._saved_message_queues: Dict[str, Deque[LbankOrderBookMessage]] = defaultdict(lambda: deque(maxlen=1000))
        self._active_order_trackers: Dict[str, LbankActiveOrderTracker] = defaultdict(LbankActiveOrderTracker)
    ...
    async def _order_book_diff_router(self):
        ...
        while True:
            try:
                ob_message: LbankOrderBookMessage = await self._order_book_diff_stream.get()
    ...
    async def _track_single_book(self, trading_pair: str):
        past_diffs_window: Deque[LbankOrderBookMessage] = deque()
        self._past_diffs_windows[trading_pair] = past_diffs_window

        message_queue: asyncio.Queue = self._tracking_message_queues[trading_pair]
        order_book: OrderBook = self._order_books[trading_pair]
        active_order_tracker: LbankActiveOrderTracker = self._active_order_trackers[trading_pair]
        ...
        while True:
            try:
                message: LbankOrderBookMessage = None
                saved_messages: Deque[LbankOrderBookMessage] = self._saved_message_queues[trading_pair]
                ...
                if message.type is OrderBookMessageType.DIFF:
                    ...
                    past_diffs: List[LbankOrderBookMessage] = list(past_diffs_window)
