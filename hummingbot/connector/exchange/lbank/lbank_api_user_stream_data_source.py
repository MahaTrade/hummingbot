...
LBANK_API_ENDPOINT = "https://api.lbkex.com/"
LBANK_USER_STREAM_ENDPOINT = "/api/v1/bullet-private"

LBANK_PRIVATE_TOPICS = [
...
class LbankAPIUserStreamDataSource(UserStreamTrackerDataSource):
    ...
    def __init__(self, lbank_auth: LbankAuth):
        self._current_listen_key = None
        self._current_endpoint = None
        super().__init__()
        self._lbank_auth: LbankAuth = lbank_auth
    ...
    async def get_listen_key(self):
        async with aiohttp.ClientSession() as client:
            header = self._lbank_auth.add_auth_to_params("POST", LBANK_USER_STREAM_ENDPOINT)
            async with client.post(f"{LBANK_API_ENDPOINT}{LBANK_USER_STREAM_ENDPOINT}", headers=header) as response:
                response: aiohttp.ClientResponse = response
                if response.status != 200:
                    raise IOError(f"Error fetching Lbank user stream listen key. HTTP status is {response.status}.")
                data: Dict[str, str] = await response.json()
                return data

    async def _subscribe_topic(self, ws: websockets.WebSocketClientProtocol) -> AsyncIterable[str]:
        try:
            for topic in LBANK_PRIVATE_TOPICS:
                subscribe_request = {
                    "id": int(time.time()),
                    "type": "subscribe",
                    "topic": topic,
