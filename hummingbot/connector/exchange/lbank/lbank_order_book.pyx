...
cdef class LbankOrderBook(OrderBook):
    @classmethod
    ...
    def snapshot_message_from_exchange(cls,
        ...
        if metadata:
            msg.update(metadata)
        return LbankOrderBookMessage(OrderBookMessageType.SNAPSHOT, {
    ...
    def diff_message_from_exchange(cls,
        ...
        if metadata:
            msg.update(metadata)
        return LbankOrderBookMessage(OrderBookMessageType.DIFF, {
    ...
    def diff_message_from_db(cls, record: RowProxy, metadata: Optional[Dict] = None) -> OrderBookMessage:
        ts = int(record["timestamp"])
        msg = ujson.loads(record["json"])  # Lbank json in DB is TEXT
        if metadata:
            msg.update(metadata)
