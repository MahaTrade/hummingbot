...
from hummingbot.connector.exchange.lbank.lbank_order_book import LbankOrderBook
...
SNAPSHOT_REST_URL = "https://api.lbkex.com/v2/depth.do"
DIFF_STREAM_URL = ""
TICKER_PRICE_CHANGE_URL = "https://api.lbkex.com/v2/ticker.do?symbol=all"
EXCHANGE_INFO_URL = "https://api.lbkex.com/v2/currencyPairs.do"
...
class LbankWSConnectionIterator:
    """
    A message iterator that automatically manages the auto-ping requirement from Lbank, and returns all JSON-decoded
    messages from a Lbank websocket connection

    Instances of this class are intended to be used with an `async for msg in <iterator>: ...` block. The iterator does
    the following:

     1. At the beginning of the loop, connect to Lbank's public websocket data stream, and subscribe to topics matching
        its constructor arguments.
     2. Start an automatic ping background task, to keep the websocket connection alive.
     3. Yield any messages received from Lbank, after JSON decode. Note that this means all messages, include ACK and
        PONG messages, are returned.
     4. Raises `asyncio.TimeoutError` if no message have been heard from Lbank for more than
    ...
    async def get_ws_connection_context() -> WSConnectionContext:
        async with aiohttp.ClientSession() as session:
            async with session.post('https://api.lbank.com/api/v1/bullet-public', data=b'') as resp:
                response: aiohttp.ClientResponse = resp
                if response.status != 200:
                    raise IOError(f"Error fetching Lbank websocket connection data."
    ...
    async def update_subscription(ws: websockets.WebSocketClientProtocol,
                                  ...
                                  subscribe: bool):
        # Lbank has a limit of 100 subscription per 10 seconds
    ...
    async def subscribe(self, stream_type: StreamType, trading_pairs: Set[str]):
        await LbankWSConnectionIterator.update_subscription(self.websocket, stream_type, trading_pairs, True)

    async def unsubscribe(self, stream_type: StreamType, trading_pairs: Set[str]):
        await LbankWSConnectionIterator.update_subscription(self.websocket, stream_type, trading_pairs, False)
    ...
    async def __aiter__(self) -> AsyncIterable[Dict[str, any]]:
        ...
        # Get connection info and connect to Lbank websocket.
...
class LbankAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    class TaskEntry:
        ...
        def __init__(self, trading_pairs: Set[str], task: asyncio.Task):
            ...
            self._message_iterator: Optional[LbankWSConnectionIterator] = None
        ...
        @property
        def message_iterator(self) -> Optional[LbankWSConnectionIterator]:
            return self._message_iterator

        @message_iterator.setter
        def message_iterator(self, msg_iter: LbankWSConnectionIterator):
    ...
    def __init__(self, trading_pairs: List[str], auth: LbankAuth = None):
        super().__init__(trading_pairs)
        self._auth = auth
        self._order_book_create_function = lambda: OrderBook()
        self._tasks: DefaultDict[StreamType, Dict[int, LbankAPIOrderBookDataSource.TaskEntry]] = defaultdict(dict)
    ...
    async def get_last_traded_prices(cls, trading_pairs: List[str]) -> Dict[str, float]:
        ...
        for trading_pair in trading_pairs:
            print('307', trading_pair)
            resp_record = [o["ticker"] for o in resp_json["data"] if convert_from_exchange_trading_pair(o["symbol"]) == trading_pair][0]
            results[trading_pair] = float(resp_record["latest"])
        return results
    ...
    async def fetch_trading_pairs() -> List[str]:
        ...
        if response:
            try:
                ...
                return [convert_from_exchange_trading_pair(item) for item in all_trading_pairs]
            except Exception:
                pass
            # Do nothing if the request fails -- there will be no autocomplete for LBank trading pairs
        return []

    @staticmethod
    async def get_snapshot(client: aiohttp.ClientSession, trading_pair: str, auth: LbankAuth = None) -> Dict[str, Any]:
        print(trading_pair)
    ...
    async def get_new_order_book(self, trading_pair: str) -> OrderBook:
        async with aiohttp.ClientSession() as client:
            ...
            snapshot_msg: OrderBookMessage = LbankOrderBook.snapshot_message_from_exchange(
                snapshot,
                snapshot_timestamp,
                metadata={"symbol": trading_pair}
            )
            order_book: OrderBook = self.order_book_create_function()
            active_order_tracker: LbankActiveOrderTracker = LbankActiveOrderTracker()
    ...
    async def _collect_and_decode_messages_loop(self, stream_type: StreamType, task_index: int, output: asyncio.Queue):
        while True:
            try:
                lbank_msg_iterator: LbankWSConnectionIterator = LbankWSConnectionIterator(
                    stream_type, self._tasks[stream_type][task_index].trading_pairs
                )
                self._tasks[stream_type][task_index].message_iterator = lbank_msg_iterator
                async for raw_msg in lbank_msg_iterator:
                    msg_type: str = raw_msg.get("type", "")
                    if msg_type in {"ack", "welcome", "pong"}:
                        pass
                    elif msg_type == "message":
                        if stream_type == StreamType.Depth:
                            order_book_message: OrderBookMessage = LbankOrderBook.diff_message_from_exchange(raw_msg)
                        else:
                            trading_pair: str = convert_to_exchange_trading_pair(raw_msg["data"]["symbol"])
                            data = raw_msg["data"]
                            order_book_message: OrderBookMessage = \
                                LbankOrderBook.trade_message_from_exchange(
                                    data,
                                    metadata={"trading_pair": trading_pair}
                                )
                        output.put_nowait(order_book_message)
                    elif msg_type == "error":
                        self.logger().error(f"WS error message from Lbank: {raw_msg}")
                    else:
                        self.logger().warning(f"Unrecognized message type from Lbank: {msg_type}. "
                                              f"Message = {raw_msg}.")
