...
from hummingbot.connector.exchange.scallop.scallop_global import ScallopGlobal
from hummingbot.connector.exchange.scallop.scallop_order_book_tracker import ScallopOrderBookTracker
from hummingbot.connector.exchange.scallop.scallop_user_stream_tracker import ScallopUserStreamTracker
# from hummingbot.connector.exchange.scallop.scallop_auth import ScallopAuth
from hummingbot.connector.exchange.scallop.scallop_in_flight_order import ScallopInFlightOrder
from hummingbot.connector.exchange.scallop import scallop_utils
...
class ScallopExchange(ExchangeBase):
    """
    ScallopExchange connects with scallop.com exchange and provides order book pricing, user account tracking and
    ...
    def __init__(self,
                 scallop_api_key: str,
                 scallop_secret_key: str,
                 trading_pairs: Optional[List[str]] = None,
                 trading_required: bool = True
                 ):
        """
        :param key: The API key to connect to private scallop.com APIs.
        ...
        self._global = ScallopGlobal(scallop_api_key, scallop_secret_key)
        # self._rest_api = ScallopRestApi(self._scallop_auth, self._http_client)
        self._order_book_tracker = ScallopOrderBookTracker(trading_pairs=trading_pairs)
        self._user_stream_tracker = ScallopUserStreamTracker(self._global, trading_pairs)
        self._ev_loop = asyncio.get_event_loop()
        self._poll_notifier = asyncio.Event()
        self._last_timestamp = 0
        self._in_flight_orders: Dict[str, ScallopInFlightOrder] = {}  # Dict[client_order_id:str, ScallopInFlightOrder]
    ...
    @property
    def in_flight_orders(self) -> Dict[str, ScallopInFlightOrder]:
    ...
    def restore_tracking_states(self, saved_states: Dict[str, any]):
        ...
        self._in_flight_orders.update({
            key: ScallopInFlightOrder.from_json(value)
        ...
        while True:
            try:
                ...
                self.logger().network(f"Unexpected error while fetching trading rules. Error: {str(e)}",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch new trading rules from scallop.com. "
                                                      "Check network connection.")
                await asyncio.sleep(0.5)

    async def _update_trading_rules(self):
        instruments_info = await self._global.rest_api.request("get", path_url="symbols")
    ...
    def _format_trading_rules(self, instruments_info: Dict[str, Any]) -> Dict[str, TradingRule]:
        ...
        for rule in instruments_info["symbols"]:
            try:
                trading_pair = scallop_utils.convert_from_exchange_trading_pair(rule["baseAsset"], rule["quoteAsset"])
                price_decimals = Decimal(str(rule["pricePrecision"]))
                quantity_decimals = Decimal(str(rule["quantityPrecision"]))
    ...
    def buy(self, trading_pair: str, amount: Decimal, order_type=OrderType.MARKET,
        ...
        """
        order_id: str = scallop_utils.get_new_client_order_id(True, trading_pair)
    ...
    def sell(self, trading_pair: str, amount: Decimal, order_type=OrderType.MARKET,
        ...
        :returns A new internal order id
        """
        order_id: str = scallop_utils.get_new_client_order_id(False, trading_pair)
    ...
    async def _create_order(self,
        ...
        symbol = scallop_utils.convert_to_exchange_trading_pair(trading_pair)
        ...
        try:
            order_result = await self._global.rest_api.request("post", "order", api_params, True)
            ...
            self.logger().network(
                f"Error submitting {trade_type.name} {order_type.name} order to Scallop for "
                f"{amount} {trading_pair} "
                f"{price}.",
                exc_info=True,
    ...
    def start_tracking_order(self,
        ...
        """
        self._in_flight_orders[order_id] = ScallopInFlightOrder(
    ...
    async def _execute_cancel(self, o: ScallopInFlightOrder) -> str:
        """
        ...
        try:
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg=f"Failed to cancel the order {o.exchange_order_id} on Scallop. "
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching account updates.",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch account updates from Scallop. "
    ...
    async def _update_balances(self):
        ...
        for account in account_info["balances"]:
            ...
            totalAmount = float(account["free"]) + float(account["locked"])
            self._account_available_balances[asset_name] = Decimal(str(account["free"]))
            self._account_balances[asset_name] = Decimal(str(totalAmount))
    ...
    async def _update_order_status(self):
        ...
        if current_tick > last_tick and len(self._in_flight_orders) > 0:
            ...
            for tracked_order in tracked_orders:
                ...
                new_client_order_id = str(self.current_timestamp)
    ...
    def _process_order_status(self, exchange_order_id: str, status: int):
        ...
        #                        f"Reason: {scallop_utils.get_api_reason(order_msg['reason'])}")
        #     self.trigger_event(MarketEvent.OrderFailure,
        #                        MarketOrderFailureEvent(
        #                            self.current_timestamp,
        #                            client_order_id,
    ...
    def _process_order_message_traded(self, order_msg):
        tracked_order: ScallopInFlightOrder = self.find_exchange_order(order_msg['id'])
        if tracked_order is None:
            return
    ...
    async def cancel_all(self, timeout_seconds: float):
        ...
        try:
            ...
            #         {"instrument_name": scallop_utils.convert_to_exchange_trading_pair(trading_pair)},
            #         True
            #     )
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg="Failed to cancel all orders on Scallop. Check API key and network connection."
            )
        ...
        while True:
            try:
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg="Could not fetch user events from Scallop. Check API key and network connection."
                )
    ...
    async def _user_stream_event_listener(self):
        ...
        ScallopAPIUserStreamDataSource.
        """
    ...
    async def get_open_orders(self) -> List[OpenOrder]:
        ...
        for order in result["data"]:
            # if scallop_utils.HBOT_BROKER_ID not in order["client_oid"]:
            #     continue
            ...
            ret_val.append(
                OpenOrder(
                    ...
                    trading_pair=scallop_utils.convert_from_exchange_trading_pair(order["symbol"]),
