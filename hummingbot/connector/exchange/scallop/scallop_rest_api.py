...
class ScallopRestApi:

    def __init__(self, auth: ScallopAuth, http_client_getter: Callable[[], aiohttp.ClientSession]):
        self._auth = auth
    ...
    async def request(self,
        ...
        """
        path_url = f"/sapi/v1/{path_url}"
        url = f"{Constants.REST_URL}{path_url}"
        client = await self._http_client()
        if is_auth_required:
            headers = self._auth.get_private_headers(method, path_url, params)
        else:
            headers = {}
        ...
        if method == "get":
            ...
            # headers["X-CH-TS"] = str(headers["X-CH-TS"])
        ...
        if 'code' in dict.keys(parsed_response):
