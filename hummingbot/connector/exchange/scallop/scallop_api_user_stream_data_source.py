...
# from .scallop_auth import ScallopAuth
from hummingbot.connector.exchange.scallop.scallop_websocket import ScallopWebsocket
...
class ScallopAPIUserStreamDataSource(UserStreamTrackerDataSource):
    ...
    def __init__(self, _global: ScallopGlobal, trading_pairs: Optional[List[str]] = []):
        self._global: ScallopGlobal = _global
    ...
    async def _listen_to_orders_trades_balances(self) -> AsyncIterable[Any]:
        ...
        try:
            ws = ScallopWebsocket(self._global.auth)
            await ws.connect()
        ...
        *required
        ...
        while True:
            try:
                ...
                self.logger().error(
                    "Unexpected error with Scallop WebSocket connection. " "Retrying after 30 seconds...", exc_info=True
                )
