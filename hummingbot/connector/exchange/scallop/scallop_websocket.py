...
from hummingbot.connector.exchange.scallop.scallop_utils import RequestId
...
class ScallopWebsocket(RequestId):
    ...
    def __init__(self, auth: Optional[ScallopAuth] = None):
        self._auth: Optional[ScallopAuth] = auth
    ...
    async def _messages(self) -> Any:
        try:
            ...
            self.logger().exception('scallop.websocket._messages', stack_info=True)
            raise
        finally:
            if not success:
