...
from hummingbot.connector.connector.quickswap.quickswap_in_flight_order import QuickswapInFlightOrder
from hummingbot.client.settings import GATEAWAY_CA_CERT_PATH, GATEAWAY_CLIENT_CERT_PATH, GATEAWAY_CLIENT_KEY_PATH
from hummingbot.client.config.global_config_map import global_config_map
from hummingbot.core.utils.polygon import check_transaction_exceptions, fetch_trading_pairs
...
class QuickswapConnector(ConnectorBase):
    """
    QuickswapConnector connects with quickswap gateway APIs and provides pricing, user account tracking and trading
    functionality.
    """
    ...
    def __init__(self,
        ...
        :param polygon_rpc_url: this is usually infura RPC URL
        ...
        self._polygon_rpc_url = polygon_rpc_url
    ...
    async def initiate_pool(self) -> str:
        ...
        while True:
            try:
                # self.logger().info(f"Initializing Quickswap connector and paths for {self._trading_pairs} pairs.")
                resp = await self._api_request("get", "pol/quickswap/start",
    ...
    async def auto_approve(self):
        """
        Automatically approves Quickswap contract as a spender for token in trading pairs.
        ...
        for token, amount in self._allowances.items():
            if amount <= s_decimal_0:
                amount_approved = await self.approve_quickswap_spender(token)
    ...
    async def approve_quickswap_spender(self, token_symbol: str) -> Decimal:
        """
        Approves Quickswap contract as a spender for a token.
        ...
        if amount_approved > 0:
            self.logger().info(f"Approved Quickswap spender contract for {token_symbol}.")
        else:
            self.logger().info(f"Quickswap spender contract approval failed on {token_symbol}.")
        return amount_approved

    async def get_allowances(self) -> Dict[str, Decimal]:
        """
        Retrieves allowances for token in trading_pairs
        :return: A dictionary of token and its allowance (how much Quickswap can spend).
        """
        ret_val = {}
        resp = await self._api_request("post", "pol/allowances",
        ...
        try:
            ...
            if any(item not in resp.keys() for item in required_items):
                ...
                if price is not None and len(exceptions) == 0:
                    ...
                    fee_overrides_config_map["quickswap_maker_fee_amount"].value = Decimal(str(gas_cost))
                    fee_overrides_config_map["quickswap_taker_fee_amount"].value = Decimal(str(gas_cost))
    ...
    async def _create_order(self,
        ...
        try:
            order_result = await self._api_request("post", "pol/quickswap/trade", api_params)
            ...
            self.logger().network(
                f"Error submitting {trade_type.name} order to Quickswap for "
    ...
    def start_tracking_order(self,
        ...
        """
        self._in_flight_orders[order_id] = QuickswapInFlightOrder(
    ...
    @property
    def in_flight_orders(self) -> Dict[str, QuickswapInFlightOrder]:
