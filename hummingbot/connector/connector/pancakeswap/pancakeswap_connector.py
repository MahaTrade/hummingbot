...
from hummingbot.connector.connector.pancakeswap.pancakeswap_in_flight_order import PancakeswapInFlightOrder
from hummingbot.client.settings import GATEAWAY_CA_CERT_PATH, GATEAWAY_CLIENT_CERT_PATH, GATEAWAY_CLIENT_KEY_PATH
from hummingbot.client.config.global_config_map import global_config_map
from hummingbot.core.utils.bsc_chain import check_transaction_exceptions, fetch_trading_pairs
...
class PancakeswapConnector(ConnectorBase):
    """
    PancakeswapConnector connects with pancakeswap gateway APIs and provides pricing, user account tracking and trading
    functionality.
    """
    ...
    def __init__(self,
        ...
        :param bsc_rpc_url: this is usually infura RPC URL
    ...
    async def auto_approve(self):
        """
        Automatically approves Pancakeswap contract as a spender for token in trading pairs.
        ...
        for token, amount in self._allowances.items():
            if amount <= s_decimal_0:
                amount_approved = await self.approve_pancakeswap_spender(token)
    ...
    async def approve_pancakeswap_spender(self, token_symbol: str) -> Decimal:
        """
        Approves Pancakeswap contract as a spender for a token.
        ...
        if amount_approved > 0:
            self.logger().info(f"Approved Pancakeswap spender contract for {token_symbol}.")
        else:
            self.logger().info(f"Pancakeswap spender contract approval failed on {token_symbol}.")
        ...
        try:
            ...
            if any(item not in resp.keys() for item in required_items):
                ...
                if price is not None and len(exceptions) == 0:
                    ...
                    fee_overrides_config_map["pancakeswap_maker_fee_amount"].value = Decimal(str(gas_cost))
                    fee_overrides_config_map["pancakeswap_taker_fee_amount"].value = Decimal(str(gas_cost))
    ...
    async def _create_order(self,
        ...
        try:
            ...
            self.logger().network(
                f"Error submitting {trade_type.name} order to Pancakeswap for "
    ...
    def start_tracking_order(self,
        ...
        """
        self._in_flight_orders[order_id] = PancakeswapInFlightOrder(
    ...
    @property
    def in_flight_orders(self) -> Dict[str, PancakeswapInFlightOrder]:
